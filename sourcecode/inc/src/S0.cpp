...
vector<vector<int>> W25ind(P);
vector<vector<int>> W26ind(P);
...
void Dualsetup(){ 
	...
	for (int p=0;p!=P;++p){  
		W25ind[p].resize(WND[p],-1);
		W26ind[p].resize(WND[p],-1);
	}
	for (int p=0;p!=P;++p){ 
		C4N[p]=0;  
		for (int b=0;b!=BN;++b){
	...
	for (int p=0;p!=P;++p) { 
		...
		for (int nn=1;nn!=PTN[p];++nn){  
			for (int i=0;i!=I2;++i){  
				...
				int wind1=WNDindex[i][nn];  
				W25ind[p][wind1]=C7N[p];
...
double TRP (int SCE, int PH){
	...
	IloNumVarArray w(env, WND[PH], 0, IloInfinity, ILOFLOAT);
	...
	try{
		...
		{ 
			...
			for (int nn=0;nn!=WND[PH];++nn){
				Z5+=LL*w[nn];
			}
			model.add(obj==Z2+Z3+Z4+Z5+F);
			Z2.end();
			Z3.end();
			Z4.end();
			Z5.end();
		...
		}
		{
			for (int nn=1;nn!=PTN[PH];++nn){ 
				for (int i=0;i!=I2;++i){  
					...
					model.add(u[ndind1]-v[ndind1]+w[wind1]==u[ndind2]-v[ndind2]+D[SCE][nd1]+tsum1+sum1); 
					tsum1.end();
					sum1.end();
				}
			}
		...
		{ 
			for (int i=0;i!=I2;++i){
				...
				model.add(u[ndind1]-v[ndind1]+w[wind1]==tu[i+I1]-tv[i+I1]+D[SCE][nd1]+tsum1+sum1); 
				tsum1.end();
				sum1.end();
			}
		}
...
double SepCGP (int TD, int SCE, int PH, int TSC){
	...
	try{
		...
		{   
			...
			model.add(obj==Z23+Z24+Z25+Z26+Z27+Z28+Z29+Z30+Z31+BDZ);
			Z23.end();
			Z24.end();
			Z25.end();
			Z26.end();
		...
		}
		{
			for (int nn=0;nn!=WND[PH];++nn){
				IloExpr sum1(env);
				int thec25ind=W25ind[PH][nn];
				if (thec25ind!=-1){
					sum1+=d25[thec25ind];
				}
				int thec26ind=W26ind[PH][nn];
				if (thec26ind!=-1){
					sum1+=d26[thec26ind];
				}
				model.add(sum1<=LL);
