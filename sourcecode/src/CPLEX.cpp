...
void CPLEX () {
	...
	IloEnv env;
	IloCplex cplex(env);
	cplex.setParam(IloCplex::Param::TimeLimit,1800.0*P);
	...
	try{
		...
		}
		{
			cplex.extract(model);	
			cplex.solve();
			mycout01<<"The model gap is "<<cplex.getMIPRelativeGap()*100<<endl;
			double thecost=cplex.getObjValue();
			mycout01<<"The result is "<<thecost<<endl;
			double bidcost=0;
			double IHcost=0;
			double TRcost=0;
			for (b=0;b!=BN;++b){
...
int main (int argc, char **argv) {
	...
	mycout01<<"Time:"<<((t_end.tv_sec - t_start.tv_sec)*1000000+(t_end.tv_usec - t_start.tv_usec))/double(1000000)<<endl;
