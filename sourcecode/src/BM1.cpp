...
double STivvol[TTRD];    
double STbgvol[TTRD];    
double STivvol_s[TTRD];    
double STbgvol_s[TTRD];    
double STivvol_d[TTRD];    
double STbgvol_d[TTRD];    
...
double BIDP (int record, int LCI) { 
	...
	IloNumVarArray x(env, BN, 0, 0, ILOFLOAT); 
...
double INTBIDP (int record,double timlim) { 
	...
	IloEnv env; 
	IloNumVar obj(env, -IloInfinity, IloInfinity, ILOFLOAT);
	IloNumVarArray x(env, BN, 0, 0, ILOINT); 
	IloNumVarArray y(env, BN, 0, IloInfinity, ILOFLOAT); 
	IloNumVar F(env, 0, IloInfinity, ILOFLOAT); 
	IloArray<IloNumVarArray> z(env, STTSN[0]);
	IloArray<IloNumVarArray> u(env, STTSN[0]);
		...
		
		{
			...
			for (int nn=0;nn!=PNN[PH];++nn){
				...
				if(Nodeset[0][thend]<I1){
					STivvol_s[SCE]+=theval;
				}
				else{
					STivvol_d[SCE]+=theval;
				}
			...
			for (int nn=0;nn!=PNN[PH];++nn){
				...
				STbgvol[SCE]+=theval;
				if(Nodeset[0][thend]<I1){
					STbgvol_s[SCE]+=theval;
				}
				else{
					STbgvol_d[SCE]+=theval;
				}
...
#pragma omp section
	...
	for (int nn=0;nn!=TTRD;++nn){
		...
		TTivvol+=STivvol[nn];
		TTbgvol+=STbgvol[nn];
		TTivvol_s+=STivvol_s[nn];
		TTivvol_d+=STivvol_d[nn];
		TTbgvol_s+=STbgvol_s[nn];
		TTbgvol_d+=STbgvol_d[nn];
	}
